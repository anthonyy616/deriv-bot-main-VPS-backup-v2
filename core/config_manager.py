...
def get_default_symbol_config() -> Dict[str, Any]:
    """
    Default configuration for a single symbol/asset
    
    - max_pairs: Number of grid levels (1,3,5,7,9) = 1 center + symmetric above/below
    - max_positions: Number of trades per pair (1-20), determines lot_sizes length
    """
    return {
        "enabled": False,
        "spread": 20.0,
        "max_pairs": 5,          # Grid levels: 1, 3, 5, 7, or 9
        "max_positions": 5,      # Trades per pair: 1-20
        "lot_sizes": [0.01, 0.01, 0.01, 0.01, 0.01],  # Length = max_positions
        "buy_stop_tp": 50.0,
        "buy_stop_sl": 75.0,
        "sell_stop_tp": 50.0,
        "sell_stop_sl": 75.0,
        ...
        if "symbols" in new_config:
            for symbol, sym_cfg in new_config["symbols"].items():
                if symbol in self.config["symbols"]:
                    ...
                    # Validate max_pairs: must be 1, 3, 5, 7, or 9
                    max_pairs = self.config["symbols"][symbol].get("max_pairs", 5)
                    valid_pairs = [1, 3, 5, 7, 9]
                    if max_pairs not in valid_pairs:
                        # Find closest valid value
                        max_pairs = min(valid_pairs, key=lambda x: abs(x - max_pairs))
                        self.config["symbols"][symbol]["max_pairs"] = max_pairs
                    
                    # Validate max_positions: 1-20 (trades per pair)
                    max_pos = self.config["symbols"][symbol].get("max_positions", 5)
                    max_pos = max(1, min(20, max_pos))  # Clamp to 1-20
                    self.config["symbols"][symbol]["max_positions"] = max_pos
                    
                    # Ensure lot_sizes length matches max_positions
                    lot_sizes = self.config["symbols"][symbol].get("lot_sizes", [])
                    
                    # Adjust lot_sizes array length
                    if len(lot_sizes) < max_pos:
                        # Extend with 0.01 defaults
                        lot_sizes.extend([0.01] * (max_pos - len(lot_sizes)))
                    elif len(lot_sizes) > max_pos:
                        lot_sizes = lot_sizes[:max_pos]
                    
                    self.config["symbols"][symbol]["lot_sizes"] = lot_sizes
